...
static void ArrayDouble(Array *array){
  ...
  if (new_data_ptr == NULL){ perror("realloc command failed"); ArrayDestroy(array); exit(EXIT_FAILURE); }
...
void ArrayAppend(Array *array, const float input){
  ...
  if (array->index + 1 >= array->length){ ArrayDouble(array); }
...
void ArrayAppendPointer(Array *array, const float *input, const size_t pointerLength){
  ...
  if (array->index + pointerLength - 1 >= array->length){ ArrayDouble(array); }
